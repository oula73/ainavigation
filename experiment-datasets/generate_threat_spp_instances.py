...
import sys
sys.path.append(r"./experiment-datasets")
from data import load_maze_from_directory, get_goalMaps_threatMaps_optPolicies_optDists
...
def generate_data(input_path, train_size, valid_size, test_size, mechanism,
    ...
    for i, (split, num_images) in enumerate(
        ...
        if tile_size == 1:
            ...
            mazes[i] = tmp_montage
        goal[i], threaten[i], opt_policies[i], opt_dists[
            i] = get_goalMaps_threatMaps_optPolicies_optDists(mazes[i],
                                                   mechanism,
                                                   from_largest=True,
                                                   edge_size=edge_size,
