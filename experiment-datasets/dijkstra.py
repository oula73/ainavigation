"""Dijkstra search
This script has been copied from https://github.com/RLAgent/gated-path-planning-networks
"""
...
def dijkstra_dist(maze, mechanism, goal, threat_map):
        ...
        for y in range(maze.shape[0]):
            for x in range(maze.shape[1]):
                if (orient == goal[0]) and (y == goal[1]) and (x == goal[2]):
                    continue
                pq.insert(
                    (orient, y, x), mechanism.num_orient * maze.shape[0] * maze.shape[1] + threat_map.max()
                )
    ...
    while not pq.empty():
        ...
        for n in mechanism.invneighbors_func(maze, p_orient, p_y, p_x):
            ...
            if n[1] - p_y == 0 or n[2] - p_x == 0:
                curr_to_n = val + 1 + threat_map[p_y][p_x]
            else:
                curr_to_n = val + np.sqrt(2) + threat_map[p_y][p_x]
