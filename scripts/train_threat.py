...
from train_utils.threat_planner import PlannerModule, set_global_seeds
from pytorch_lightning.callbacks import ModelCheckpoint

@hydra.main(config_path="config", config_name="train_threat")
def main(config):

    # dataloaders
    set_global_seeds(config.seed)
    train_loader = create_threat_dataloader(
        config.dataset + ".npz", "train", config.params.batch_size, shuffle=True
    )
    val_loader = create_threat_dataloader(
