...
class PlannerModule(pl.LightningModule):
    ...
    def forward(self, map_designs, start_maps, goal_maps, threat_maps):
        return self.planner(map_designs, start_maps, goal_maps, threat_maps)

    def configure_optimizers(self) -> torch.optim.Optimizer:
        return torch.optim.RMSprop(self.planner.parameters(), self.config.params.lr)

    def training_step(self, train_batch, batch_idx):
        map_designs, start_maps, goal_maps, threat_maps, opt_trajs = train_batch
        outputs = self.forward(map_designs, start_maps, goal_maps, threat_maps)
        loss = nn.L1Loss()(outputs.histories + outputs.paths * threat_maps, opt_trajs + opt_trajs * threat_maps)
        self.log("metrics/train_loss", loss)

        return loss

    def validation_step(self, val_batch, batch_idx):
        map_designs, start_maps, goal_maps, threat_maps, opt_trajs = val_batch
        outputs = self.forward(map_designs, start_maps, goal_maps, threat_maps)
        loss = nn.L1Loss()(outputs.histories + outputs.paths * threat_maps, opt_trajs + opt_trajs * threat_maps)
        ...
        if map_designs.shape[1] == 1:
            va_outputs = self.vanilla_astar(map_designs, start_maps, goal_maps, threat_maps)
